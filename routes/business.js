...
router.post('/addteam',requireLogin, wrapAsync(async(req,res)=>{
    const{username, perHour, email, password=process.env.RANDOM_STR}= req.body;
...
router.get('/insights',requireLogin, wrapAsync(async(req,res)=>{
    ...
    for(let employee of teams.employees){
        ...
        // } 
        // teamList.push({employee: user.username, workTime: totalWork, timeCard: user.workHour.timeCard})
        // totalWork = 0;
    }
